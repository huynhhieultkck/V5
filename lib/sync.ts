...
export async function syncResellStock() {
  console.log("[Sync] Bắt đầu đồng bộ kho hàng Resell...");
  ...
  try {
    ...
    for (const product of resellProducts) {
      ...
      console.log(`[Sync] Đã cập nhật sản phẩm ID ${product.id}: ${currentStock} còn lại.`);
    }

    console.log("[Sync] Hoàn tất đồng bộ.");
  } catch (error) {
    console.error("[Sync] Lỗi nghiêm trọng khi đồng bộ:", error);
  }
}